...
/** OTA panel updates from panel_ota.json on your server. */
public final class RebelOtaManager {
    ...
    public static void checkAndUpdate(final Context ctx, final boolean force, final Callback cb) {
        new Thread(() -> {
            ...
            try {
                String manifestUrl = RebelPanelPaths.otaManifestUrl(ctx);
                if (manifestUrl == null || manifestUrl.isEmpty()) {
                    main.post(cb::onNoUpdate);
                    return;
                }
                String raw = httpGet(manifestUrl + (manifestUrl.contains("?") ? "&" : "?") + "t="
    ...
    private static String httpGet(String urlStr) throws Exception {
        ...
        c.setRequestProperty("User-Agent", "NyaPanel-OTA/" + BuildConfig.VERSION_CODE);
    ...
    private static void downloadTo(String urlStr, File dest) throws Exception {
        ...
        if (dest.exists() && !dest.delete()) {
            throw new Exception("Cannot replace " + dest.getName());
        }
        if (!tmp.renameTo(dest)) {
            throw new Exception("Cannot save " + dest.getName());
        }
    }
}
