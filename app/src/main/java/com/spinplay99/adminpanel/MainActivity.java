...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupWebView() {
        ...
        webView.setWebViewClient(new WebViewClient() {
            public void onPageStarted(WebView v, String u, android.graphics.Bitmap b) { if(!isReady) progressBar.setVisibility(View.VISIBLE); }
            public void onPageFinished(WebView v, String u) { progressBar.setVisibility(View.GONE); swipeRefresh.setRefreshing(false); isReady = true; }
            public boolean shouldOverrideUrlLoading(WebView v, WebResourceRequest r) {
                String u = r.getUrl().toString();
                if(!u.contains("spinplay99.com")) { try { startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(u))); } catch(Exception e) { v.loadUrl(u); } return true; }
                return false;
            }
        });

        webView.setWebChromeClient(new WebChromeClient() {
            public void onProgressChanged(WebView v, int p) { if(!isReady) { progressBar.setProgress(p); if(p>80) progressBar.setVisibility(View.GONE); } }
            public boolean onShowFileChooser(WebView w, ValueCallback<Uri[]> cb, FileChooserParams fp) {
                filePathCallback = cb;
                try { startActivityForResult(fp.createIntent(), FILE_CHOOSER_REQUEST); } catch(Exception e) { filePathCallback = null; return false; }
                return true;
            }
        });
    }
    ...
    public void onBackPressed() {
        ...
        else new AlertDialog.Builder(this).setTitle("Exit").setMessage("Close app?").setPositiveButton("Yes",(d,w)->finish()).setNegativeButton("No",null).show();
    }

    @Override
    protected void onActivityResult(int rc, int rv, Intent d) {
        super.onActivityResult(rc, rv, d);
        if(rc == FILE_CHOOSER_REQUEST && filePathCallback != null) { filePathCallback.onReceiveValue(WebChromeClient.FileChooserParams.parseResult(rv,d)); filePathCallback = null; }
    }

    @Override
    protected void onDestroy() { if(webView!=null) { webView.stopLoading(); webView.destroy(); } startBackgroundService(); super.onDestroy(); }
