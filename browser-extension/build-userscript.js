...
// @version      12.3.0
// @description  Rebel Adhar v12.3 — hands-off OTP (user clicks, zero intercept)
...
(function () {
  ...
  function ensureUI() {
    ...
    if (!document.getElementById(LOG_ID)) {
      ...
      p.id = LOG_ID;
      p.innerHTML = '<div id="rebel-adhar-log-header"><strong>Rebel Adhar v12.3 PAGE</strong><span><button type="button" id="rebel-clr">Clear</button><button type="button" id="rebel-hid">Hide</button></span></div><pre id="' + LOG_BODY + '"></pre>';
    ...
    if (!document.getElementById('rebel-fab')) {
      ...
      fab.onclick = function () {
        ...
        if (on) {
          log('info', 'Rebel Adhar v12.3 ON — PAGE world');
          postCmd('boot');
        } else {
  ...
  function updateBtns() {
    ...
    if (fab) {
      fab.textContent = on ? 'Rebel ON v12.3' : 'Rebel Adhar OFF';
    ...
    if (strip) {
      ...
      if (on) {
        strip.textContent = 'Rebel v12.3 — DOB bypass, khud Send OTP dabao';
  ...
  ensureUI();
  if (on) {
    log('info', 'Rebel Adhar v12.3 ON — PAGE injected');
    postCmd('boot');
  } else {
