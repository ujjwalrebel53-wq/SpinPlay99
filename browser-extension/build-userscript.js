...
// @version      12.4.5
// @description  Rebel Adhar v12.4.5 — native generateOTP first (fresh React closure)
...
(function () {
  ...
  function ensureUI() {
    ...
    if (!document.getElementById(LOG_ID)) {
      ...
      p.id = LOG_ID;
      p.innerHTML = '<div id="rebel-adhar-log-header"><strong>Rebel Adhar v12.4.5 PAGE</strong><span><button type="button" id="rebel-clr">Clear</button><button type="button" id="rebel-hid">Hide</button></span></div><pre id="' + LOG_BODY + '"></pre>';
    ...
    if (!document.getElementById('rebel-fab')) {
      ...
      fab.onclick = function () {
        ...
        if (on) {
          log('info', 'Rebel Adhar v12.4.5 ON — PAGE world');
          postCmd('boot');
        } else {
  ...
  function updateBtns() {
    ...
    if (fab) {
      fab.textContent = on ? 'Rebel ON v12.4.5' : 'Rebel Adhar OFF';
    ...
    if (strip) {
      ...
      if (on) {
        strip.textContent = 'Rebel v12.4.5 React — DOB bypass, khud Send OTP dabao';
  ...
  ensureUI();
  if (on) {
    log('info', 'Rebel Adhar v12.4.5 ON — PAGE injected');
    postCmd('boot');
  } else {
