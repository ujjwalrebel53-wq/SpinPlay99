#!/usr/bin/env python3
"""
Rebel UIDAI Telegram Bot — single file, pure HTTP API (no browser, no proxy).

Direct requests to tathya.uidai.gov.in APIs. Run on India VPS for best results.

Usage:
  cd www
  pip install -r requirements.txt
  python bot.py --test    # check API connectivity
  python bot.py
"""
...
def test_uidai_api() -> dict[str, Any]:
    """Direct POST to captcha API — no proxy, no browser."""
    headers = get_headers(new_request_id())
    payload = {'captchaLength': '6', 'captchaType': '2'}
    try:
        r = requests.post(
            IMAGE_CAPTCHA_URL, headers=headers, json=payload,
            timeout=(int(os.getenv('AADHAR_CONNECT_TIMEOUT', '8')), int(os.getenv('AADHAR_TIMEOUT', '20'))),
        )
        ...
        if ok:
            ...
            png_len = len(png)
            ok = bool(txn and png_len > 50)
        return {
            'ok': ok,
            'status': r.status_code,
            'txn': txn[:16] if txn else None,
            'png_bytes': png_len,
            'engine': BOT_ENGINE_VERSION,
        }
    except requests.RequestException as e:
        return {'ok': False, 'error': str(e), 'engine': BOT_ENGINE_VERSION}
...
class UidaiSession:
    """Pure HTTP UIDAI session — direct API calls only."""
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [
        ...
        '',
        '🌐 Pure API — direct HTTP to UIDAI (India VPS recommended)',
        '',
...
def main() -> None:
    ...
    log.info(
        'Bot start v%s — API-only direct | access: %s | owner: %s',
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] in ('--test', 'test'):
        result = test_uidai_api()
        print(json.dumps(result, indent=2))
        if result.get('ok'):
            print('✅ UIDAI captcha API reachable')
        else:
            print('❌ UIDAI API not reachable from this server (use India VPS)')
