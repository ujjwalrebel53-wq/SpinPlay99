...
.page.sub-screen .hdr-block{min-height:180px;padding:0}
.sub-hdr-grid{display:grid;grid-template-columns:150px 1fr 40px;grid-template-rows:auto auto auto;align-items:center;padding:8px 16px 0;gap:4px 8px;min-height:180px}
.sub-hdr-grid .home-btn{grid-column:1;grid-row:2;width:150px;height:40px;font-size:14px}
.sub-hdr-grid .hdr-title.big{grid-column:1/3;grid-row:1;font-size:25px;text-align:left;padding-left:0;margin-top:4px}
.sub-hdr-grid .upi-line{grid-column:2/4;grid-row:2;font-size:20px;padding:0}
.sub-hdr-grid .icon-btn{grid-column:3;grid-row:1;justify-self:end}
.sub-hdr-grid .search-row{grid-column:1/4;grid-row:3;padding:0 0 16px;margin-top:8px}
.money-hdr .hdr-title{text-align:center;padding:12px 0 0}
.money-hdr .btn-row .nya-btn{height:40px}
.money-active-row .nya-btn{flex:1}
.nya-web-tools{display:flex}
.nya-apk-shell .nya-web-tools{display:none!important}
.nya-apk-only{display:none}
.nya-apk-shell .nya-apk-only{display:block}
.sms-card{margin:2px 5px;border-radius:20px;background:var(--card-border);box-shadow:0 8px 24px rgba(0,0,0,.25);overflow:hidden}
.sms-inner{background:var(--card);padding:10px 8px 28px 8px;display:grid;grid-template-columns:60px 1fr auto;grid-template-rows:auto auto;gap:4px 10px;position:relative;min-height:72px}
.sms-icon{font-size:40px;width:60px;height:60px;display:flex;align-items:center;justify-content:center;grid-row:1;grid-column:1}
.sms-body{grid-column:2/4;grid-row:1;font-size:14px;color:var(--black);word-break:break-word;line-height:1.35;max-height:10em;overflow:hidden}
.sms-foot{display:flex;justify-content:space-between;grid-column:1/4;grid-row:2;font-size:12px;font-weight:700;color:var(--muted);padding:0 8px}
.sms-del{grid-column:3;grid-row:1;font-size:22px;padding:4px;align-self:start}
...
.nya-menu-wrap{position:relative}
.nya-menu-btn{font-size:26px;line-height:1;font-weight:900;letter-spacing:-2px}
.nya-menu-drop{position:absolute;top:100%;right:0;margin-top:4px;min-width:210px;background:#fff;border:1px solid var(--card-border);border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.22);z-index:40;display:none;overflow:hidden}
.nya-menu-drop.open{display:block}
.nya-menu-item{display:block;width:100%;padding:14px 16px;border:none;background:#fff;text-align:left;font-size:14px;font-weight:700;color:var(--black);cursor:pointer;touch-action:manipulation;border-bottom:1px solid rgba(84,107,77,.15)}
.nya-menu-item:last-child{border-bottom:none}
.nya-menu-item:active{background:var(--main)}
...
.detail-sim-row{display:flex;justify-content:center;gap:8px;padding:4px 16px 0;margin:0 auto;max-width:290px;width:100%}
.detail-sim-btn{flex:1 1 0;min-width:0;height:40px;font-size:13px;padding:0 6px}
...
.dev-card-inner{background:var(--card);height:150px;min-height:150px;max-height:150px;padding:8px 12px;display:grid;grid-template-columns:60px 1fr auto;grid-template-rows:auto auto auto auto;gap:4px 10px;position:relative;overflow:hidden}
...
.tg-login-status{margin:8px 0;padding:10px 12px;border-radius:12px;background:#f4f7f2;border:1px solid var(--card-border);font-size:12px;line-height:1.45;color:var(--black)}
.tg-login-status.pending{background:#fff8e6}
.tg-login-status.ok{background:#e8f5e9;border-color:#81c784}
.tg-login-status.bad{background:#ffebee;border-color:#e57373}
.tg-login-status a{color:var(--black);font-weight:700}
  ...
  <div class="hdr-block">
    <div class="hdr-row">
      ...
      <div class="hdr-actions">
        ...
        <div class="nya-menu-wrap">
          <button type="button" class="icon-btn nya-menu-btn" onclick="toggleNyaMenu(event)" title="Menu">⋮</button>
          <div class="nya-menu-drop" id="nyaMenuDrop">
            <button type="button" class="nya-menu-item" onclick="nyaMenuAction('server')">Panel Server</button>
            <button type="button" class="nya-menu-item" onclick="nyaMenuAction('firebase')">Firebase Projects</button>
            <button type="button" class="nya-menu-item" onclick="nyaMenuAction('autotoken')">Auto Token</button>
            <button type="button" class="nya-menu-item nya-apk-only" onclick="nyaMenuAction('ota')">Check OTA Update</button>
            <button type="button" class="nya-menu-item nya-apk-only" onclick="nyaMenuAction('reload')">Reload Panel</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="btn-row nya-web-tools">
...
<div id="view-online" class="page sub-screen">
  <div class="hdr-block">
    <div class="sub-hdr-grid">
      <button type="button" class="nya-btn home-btn" onclick="showPage('home')">Home 🏡</button>
...
<div id="view-money" class="page sub-screen">
  <div class="hdr-block money-hdr">
    <div class="hdr-title" id="moneyTotal">Total:-</div>
    <div class="btn-row">
      <button type="button" class="nya-btn wide" onclick="showPage('home')">Home 🏡</button>
      <button type="button" class="nya-btn wide" onclick="pickMoneyAmountRange()">Amount 💵</button>
    </div>
    <div class="btn-row money-active-row">
...
<div id="view-device" class="page">
  <div class="device-fixed-top">
    ...
    <div class="sim-row detail-sim-row">
      ...
      <button type="button" class="detail-btn detail-sim-btn" onclick="setupAutoTokenFromDevice()">Auto Token</button>
    </div>
...
<div class="sheet device-setup" id="autoTokenDeviceSheet">
  ...
  <div class="token-fields">
    ...
    <div class="tg-login-status pending" id="tgDeviceLoginStatus">Device SIM se Telegram login — neeche button dabao</div>
    <div class="token-btn-row">
      <button type="button" class="nya-btn" onclick="startTelegramDeviceLogin()">📱 Telegram Login</button>
    </div>
